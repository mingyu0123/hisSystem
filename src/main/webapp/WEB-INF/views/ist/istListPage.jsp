...
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
				...
				<div style="text-align: right;">
					<select id="searchType" class="form-control" style="width: 100px;display: initial;font-size: 15">
						...
						<option value="writer" <c:if test="${searchType == 'writer' }">selected="selected"</c:if>>수신인</option>
					</select>
				...
				<table id="istTable" class="table table-hover" style="margin-top:20px;">
				  <colgroup> 
					<col width="10%"/>
					<col width="*"/>
					<col width="15%"/>
				  ...
				  <tbody>
				  	<c:forEach items="${list}" var="ist" varStatus="sta">
						<tr id="tr${ist.istSeq }" onclick="fnDetail(${ist.istSeq },'${ist.istTitle}','${ist.istRecipient}','${ist.istStatus}','${ist.istContent}','${ist.istStatusday}','${ist.insDateDetail }')">
							<td>${totalCnt+(-(8*(paging.nowPage-1))-(sta.index))}</td>
							<td>${ist.istTitle }</td>
